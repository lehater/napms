...
_ERROR_DEPENDENCIES = {
    ...
    "ScopedConnectivityUnavailable": "ScopedConnectivityInventory",
}
...
def _scoped_relationship_dto(relationship) -> dict[str, Any]:
    return {
        ...
        "decision": {
            "state": relationship.decision.state.value,
        },
        "policy": {
...
def create_http_api(dependencies: HttpApiDependencies) -> FastAPI:
    ...
    def discover_scoped_connectivity_scopes(
        ...
        return {
            ...
            "scopes": [
                {"scope": scope} for scope in result.permitted_scopes
            ],
            "ambiguousScopes": [
    ...
    def read_scoped_connectivity_inventory(
        ...
        request.state.authority_reference = (
            result.page.read_authority_reference
        )
        _set_outcome(
            request,
