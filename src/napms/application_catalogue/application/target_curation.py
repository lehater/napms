...
class CreateApplicationDeployment:
    ...
    def execute(
        ...
        commit_outcome = _commit(self._catalogue)
        if commit_outcome is not None:
            return ApplicationDeploymentMutationResult(commit_outcome)
        return ApplicationDeploymentMutationResult(
            TargetMutationOutcome.CREATED,
            application_deployment=value,
        )
    ...
    def execute(
        ...
        if receipt is not None:
            ...
            return DeploymentInteractionMutationResult(
                TargetMutationOutcome.RESOLVED,
                deployment_interaction=existing,
        ...
        return DeploymentInteractionMutationResult(
            TargetMutationOutcome.CREATED,
            deployment_interaction=interaction,
            compatibility=compatibility,
            dcs_revision=revision,
        )
...
class UpdateInteractionDefinitionTraffic:
    ...
    def execute(
        ...
        dependency_sets: dict[TargetDependencyKind, list[ActiveDependencyReference]] = {
        ...
        for selection in selections:
            ...
            subject = DirectedInteractionIdentity(
                ...
                dcs_contract_revision_id=projection.current_dcs_revision_id,
            )
            dependency_sets[TargetDependencyKind.CONNECTIVITY_REQUIREMENTS].extend(
                self._requirements.find_active_references(
                    subject=subject,
                    as_of=command.effective_time,
                )
            )
            dependency_sets[TargetDependencyKind.CONNECTIVITY_DECISIONS].extend(
                self._decisions.find_active_references(
                    subject=subject,
                    as_of=command.effective_time,
                )
            )
            dependency_sets[TargetDependencyKind.ACCESS_RULES].extend(
                self._access_rules.find_active_references(
                    subject=subject,
                    as_of=command.effective_time,
                )
            )
        ...
        groups = tuple(
            group
            for kind, references in dependency_sets.items()
            if (group := _dependency_group(kind, tuple(references))) is not None
        )
        if groups:
            return InteractionDefinitionMutationResult(
                TargetMutationOutcome.DEPENDENCY_BLOCKED,
