...
def repository(connection):
    value = TransactionalPostgresApplicationCatalogueCurationRepository(connection)
    value.record_command_receipt(
        actor_id="actor-1",
        idempotency_key="request-1",
        receipt=ApplicationCatalogueCommandReceipt(
            command_kind="CreateApplication",
...
def test_receipt_insert_failure_is_known_persistence_failure_before_commit():
    ...
    with pytest.raises(CataloguePersistenceError) as caught:
        repository(connection).commit()

    assert not isinstance(caught.value, CataloguePersistenceOutcomeUnknown)
...
def test_commit_acknowledgement_failure_is_outcome_unknown():
    ...
    with pytest.raises(CataloguePersistenceOutcomeUnknown):
        repository(connection).commit()
