...
class PostgresAccessRuleDependencyQuery:
    """Owner-defined active/effective Access Rule references for ACC dependency checks."""
    ...
    def page(
        ...
        *,
        ...
        """
        joins = f"""
            FROM napms_access_policy.access_rules r
            JOIN wanted w
        ...
        try:
            ...
            rows = self._connection.execute(
                f"""
                WITH wanted(source_id, destination_id, dcs_id) AS (VALUES {values})
                SELECT DISTINCT r.rule_id::text
                  {joins}
                 WHERE {predicate}
                 ORDER BY r.rule_id::text
                 OFFSET %s LIMIT %s
                """,
