...
class ConnectivityRequirementDependencySubject:
    source_component_deployment_id: UUID
    destination_component_deployment_id: UUID
    dcs_contract_revision_id: UUID


class ConnectivityRequirementDependencyQuery(Protocol):
    def page(self, *, subjects, as_of, offset, limit) -> tuple[int, tuple[str, ...]]: ...


class ReadActiveConnectivityRequirementReferences:
    def __init__(self, *, query: ConnectivityRequirementDependencyQuery) -> None:
        self._query = query
