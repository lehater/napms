...
Conversation history is disposable execution context. The repository working tree contains current project truth, current execution state and executable checkpoints. Git history is the archive for replaced or completed material.
...
- `docs/architecture/` — current target structure and architecture constraints.
- `docs/engineering/` — current operational and implementation contracts.
...
Do not create repository archive folders, decision-history catalogs, completed audit stores, dispatchers, role state machines or multi-agent runtime unless a demonstrated current requirement needs them.
