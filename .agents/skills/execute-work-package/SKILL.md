...
This Skill orchestrates the active work package. It does not bypass `docs/process/change-lifecycle.md`: stage/gate transition remains owned by the lifecycle, and code execution is allowed only after the applicable S4/G4 authorization.

## Procedure

1. Recover in one order: root `AGENTS.md` -> `docs/plans/active/README.md` -> nearest scoped `AGENTS.md`.
2. Recover current task, lifecycle stage/state when material, goal, working set, blockers, gate and next action from the resume capsule.
...
6. Before routing work, check whether the requested action is permitted by the current stage/gate:
   - implementation work with no G4 authorization -> route to `implementation-readiness-stage.md`, not `implement-slice`;
   - transition/reopen/gate ambiguity -> load `change-lifecycle.md` and resolve the owning stage first;
   - accepted implementation slice after G4 -> `implement-slice`;
...
10. Update the resume capsule whenever current task, stage/state, working set, blocker, gate or next action materially changes. Update the full plan only when coordination state or plan structure changes.
11. Stop at the gate; do not start the next stage merely because it is planned.

## Output state

Keep visible: current task, current artifact, lifecycle stage/state when material, blockers, gate/local exit, semantic result and next action.
