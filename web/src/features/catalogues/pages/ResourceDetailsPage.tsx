...
function historyEvents(history: ResourceHistoryDto | null): HistoryEvent[] {
  ...
  for (const item of history.realizations) {
    ...
    values.push({ key: `realization-start:${item.factReference}`, at: item.validFrom, title: "Technical realization started", summary: addresses || "Technical addresses recorded", detail: `Fact ${shortId(item.factReference)} · v${item.version}` })
    if (item.validTo) values.push({ key: `realization-end:${item.factReference}`, at: item.validTo, title: "Technical realization ended", summary: addresses || "Technical addresses ended" })
  }

  for (const item of history.scopeAffiliations) {
    values.push({ key: `scope-start:${item.affiliationReference}`, at: item.validFrom, title: "Scope affiliation added", summary: item.responsibilityScope })
    if (item.validTo) values.push({ key: `scope-end:${item.affiliationReference}`, at: item.validTo, title: "Scope affiliation ended", summary: item.responsibilityScope })
  }

  for (const item of history.responsibilities) {
    values.push({ key: `responsibility-start:${item.assignmentReference}`, at: item.validFrom, title: "Responsibility assigned", summary: `${roleLabel(item.role)} · ${item.displayName}`, detail: item.contact || undefined })
    if (item.validTo) values.push({ key: `responsibility-end:${item.assignmentReference}`, at: item.validTo, title: "Responsibility ended", summary: `${roleLabel(item.role)} · ${item.displayName}` })
  }
...
function PanelTitle({ children }: { children: React.ReactNode }) {
  return <h2 className="border-b border-[var(--napms-color-border)] px-4 py-3 text-sm font-semibold text-[var(--napms-color-text-primary)]">{children}</h2>
}
...
export function ResourceDetailsPage({
  ...
  async function retireResource() {
    ...
    if (!window.confirm("Retire this resource? Historical references will be preserved.")) return
    ...
    try {
      ...
      setLifecycleError(errorFrom(caught, "Resource could not be retired.").message)
    } finally {
      setSavingLifecycle(null)
    }
  }
  ...
  async function saveRealization(event: React.FormEvent) {
    ...
    try {
      ...
      if (current) await replaceCatalogueResourceRealization(current, values, new Date().toISOString())
      else await createCatalogueResourceRealization(resourceReference, values, new Date().toISOString())
      setAddresses("")
      ...
      <div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="relative">
            <Button variant="secondary" onClick={() => setActionsOpen((value) => !value)}>Actions <ChevronDown className="size-4" aria-hidden="true" /></Button>
            {actionsOpen ? (
              <div className="absolute right-0 z-20 mt-2 w-44 overflow-hidden rounded-[var(--napms-control-radius)] border border-[var(--napms-color-border)] bg-[var(--napms-color-surface)] py-1 shadow-lg">
                {resourceActive ? <button type="button" className="block w-full px-3 py-2 text-left text-sm hover:bg-[var(--napms-color-surface-subtle)]" onClick={() => { setActionsOpen(false); setEditingName(true) }}>Rename</button> : null}
                {resourceActive ? <button type="button" className="block w-full px-3 py-2 text-left text-sm text-[var(--napms-color-danger)] hover:bg-[var(--napms-color-danger-bg)]" onClick={() => void retireResource()}>Retire</button> : null}
              </div>
            ) : null}
          </div>
        </div>
      ...
      {editingName ? (
        <Surface className="p-4">
          <form className="flex flex-col gap-3 sm:flex-row sm:items-end" onSubmit={renameResource}>
            <label className="grid min-w-0 flex-1 gap-1 text-xs font-medium text-[var(--napms-color-text-body)]">Display name<Input value={nameDraft} onChange={(event) => setNameDraft(event.target.value)} /></label>
            <div className="flex gap-2"><Button type="button" variant="secondary" onClick={() => setEditingName(false)}>Cancel</Button><Button type="submit" loading={savingLifecycle === "rename"}>Save</Button></div>
          </form>
        </Surface>
      ) : null}
      {lifecycleError ? <p className="text-sm text-[var(--napms-color-danger)]">{lifecycleError}</p> : null}
      ...
      {tab === "overview" ? (
        <div className="grid min-w-0 gap-4 xl:grid-cols-2">
          <div className="grid content-start gap-4">
            <Surface><PanelTitle>Basic information</PanelTitle><div className="p-4"><DetailRow label="Name">{currentName}</DetailRow><DetailRow label="Reference"><span className="font-mono text-xs">{detail.resource.resourceReference}</span></DetailRow><DetailRow label="Lifecycle"><StatusIndicator tone={resourceActive ? "positive" : "critical"}>{detail.resource.lifecycle}</StatusIndicator></DetailRow><DetailRow label="Current as of">{formatDate(detail.asOf)}</DetailRow></div></Surface>
            <Surface>
              <PanelTitle>Technical realization</PanelTitle>
              <div className="p-4">
                {currentRealization ? <><DetailRow label="Addresses"><div className="grid gap-1 font-mono text-xs">{currentRealization.technicalAddresses.map((item) => <span key={item.endpointReference}>{item.technicalAddress}</span>)}</div></DetailRow><DetailRow label="Effective since">{formatDate(currentRealization.validFrom)}</DetailRow></> : <p className="text-sm text-[var(--napms-color-text-secondary)]">No current realization.</p>}
                {resourceActive ? <Button className="mt-3" variant="secondary" size="sm" onClick={() => setEditingAddresses(true)}>{currentRealization ? "Replace addresses" : "Add addresses"}</Button> : null}
                {editingAddresses ? <form className="mt-3 grid gap-2" onSubmit={saveRealization}><label className="grid gap-1 text-xs font-medium">Addresses<Input value={addresses} onChange={(event) => setAddresses(event.target.value)} placeholder="10.20.30.40, 10.20.30.41" /></label>{realizationError ? <p className="text-xs text-[var(--napms-color-danger)]">{realizationError}</p> : null}<div className="flex gap-2"><Button type="button" variant="secondary" size="sm" onClick={() => setEditingAddresses(false)}>Cancel</Button><Button type="submit" size="sm" loading={savingRealization}>Save</Button></div></form> : null}
              </div>
            </Surface>
          </div>
          ...
          <div className="grid content-start gap-4">
            <Surface>
              ...
              <div className="p-4">
                <div className="flex flex-wrap gap-2">{detail.effectiveScopeAffiliations.length ? detail.effectiveScopeAffiliations.map((item) => <div key={item.affiliationReference} className="flex items-center gap-2"><Tag>{item.responsibilityScope}</Tag>{resourceActive ? <button type="button" className="text-xs text-[var(--napms-color-danger)] hover:underline" disabled={endingAffiliationReference === item.affiliationReference} onClick={() => void endScopeAffiliation(item)}>End</button> : null}</div>) : <span className="text-sm text-[var(--napms-color-text-secondary)]">No current scope.</span>}</div>
                {resourceActive ? <Button className="mt-3" variant="secondary" size="sm" onClick={() => setAddingScope(true)}><Plus className="size-3.5" aria-hidden="true" />Add scope</Button> : null}
                {addingScope ? <form className="mt-3 flex gap-2" onSubmit={addScopeAffiliation}><Input value={responsibilityScope} onChange={(event) => setResponsibilityScope(event.target.value)} placeholder="Responsibility scope" /><Button type="submit" size="sm" loading={creatingAffiliation}>Add</Button></form> : null}
            ...
            <Surface>
              ...
              <div className="p-4">
                <div className="grid gap-3">{detail.effectiveResponsibilities.length ? detail.effectiveResponsibilities.map((item) => <div key={item.assignmentReference} className="grid grid-cols-[130px_minmax(0,1fr)_auto] items-start gap-3 border-b border-[var(--napms-color-border)] pb-3 last:border-0 last:pb-0"><div className="text-xs text-[var(--napms-color-text-secondary)]">{roleLabel(item.role)}</div><div><div className="text-sm font-semibold text-[var(--napms-color-primary)]">{item.displayName}</div>{item.contact ? <div className="text-xs text-[var(--napms-color-text-secondary)]">{item.contact}</div> : null}</div>{resourceActive ? <button type="button" className="text-xs text-[var(--napms-color-danger)] hover:underline" disabled={endingResponsibilityReference === item.assignmentReference} onClick={() => void endResponsibility(item)}>End</button> : null}</div>) : <span className="text-sm text-[var(--napms-color-text-secondary)]">No current responsibility.</span>}</div>
                {resourceActive ? <Button className="mt-3" variant="secondary" size="sm" onClick={() => setAddingResponsibility(true)}><Plus className="size-3.5" aria-hidden="true" />Add responsibility</Button> : null}
                {addingResponsibility ? <form className="mt-3 grid gap-2" onSubmit={addResponsibility}><div className="grid gap-2 sm:grid-cols-2"><Select value={partyKind} onChange={(event) => setPartyKind(event.target.value as "Person" | "Team")}><option value="Team">Team</option><option value="Person">Person</option></Select><Select value={responsibilityRole} onChange={(event) => setResponsibilityRole(event.target.value as ResourceResponsibilityDto["role"])}><option value="TechnicalOwner">Technical owner</option><option value="ServiceOwner">Service owner</option><option value="OperationsContact">Operations contact</option><option value="BusinessOwner">Business owner</option></Select></div><Input value={partyReference} onChange={(event) => setPartyReference(event.target.value)} placeholder="Party reference" /><Input value={responsibilityDisplayName} onChange={(event) => setResponsibilityDisplayName(event.target.value)} placeholder="Display name" /><Input value={responsibilityContact} onChange={(event) => setResponsibilityContact(event.target.value)} placeholder="Contact (optional)" /><div className="flex gap-2"><Button type="button" variant="secondary" size="sm" onClick={() => setAddingResponsibility(false)}>Cancel</Button><Button type="submit" size="sm" loading={creatingResponsibility}>Add</Button></div></form> : null}
      ...
      {tab === "history" ? (
        <Surface>
          ...
          <div className="grid gap-3 p-4">
            {events.length ? events.map((event) => <div key={event.key} className="grid gap-1 border-l-2 border-[var(--napms-color-primary-border)] pl-4 sm:grid-cols-[170px_minmax(0,1fr)]"><div className="text-xs text-[var(--napms-color-text-secondary)]">{formatDate(event.at)}</div><div><div className="text-sm font-semibold text-[var(--napms-color-text-primary)]">{event.title}</div><div className="text-sm text-[var(--napms-color-text-body)]">{event.summary}</div>{event.detail ? <div className="text-xs text-[var(--napms-color-text-secondary)]">{event.detail}</div> : null}</div></div>) : <p className="text-sm text-[var(--napms-color-text-secondary)]">No historical facts recorded.</p>}
          </div>
        </Surface>
      ) : null}
