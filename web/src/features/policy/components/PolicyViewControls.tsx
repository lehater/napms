    ...
    >
      <div className="grid gap-4 lg:grid-cols-[minmax(220px,1fr)_minmax(260px,1fr)_auto] lg:items-end">
        <Field label="Governance scope">
          ...
          >
            <option value="">
              {loading ? "Loading scopes…" : "Select scope"}
            </option>
            {scopes.map((value) => (
        ...
        <Field
          label="As of"
          hint="The browser time is converted to an explicit offset-aware instant."
        >
          <input
            type="datetime-local"
            step="1"
        ...
        <Button type="submit" loading={running} disabled={!scope || !asOf}>
          Run view
        </Button>
      </div>
      ...
      {ambiguousScopes.length > 0 ? (
        <div className="mt-4 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
          ...
          ReadEffectiveDesiredPolicy authority and remain fail-closed.
        </div>
      ) : null}
