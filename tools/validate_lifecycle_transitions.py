...
def transition(
    *,
    ...
    current_index = len(SEMANTIC_STAGES) if current_stage == "IMPLEMENTATION" else STAGE_INDEX[current_stage]
    target_index = STAGE_INDEX[target_stage]
    if target_index >= current_index:
        raise ValueError("REOPEN must target an earlier semantic stage")

    dirty = list(SEMANTIC_STAGES[target_index + 1 : min(current_index + 1, len(SEMANTIC_STAGES))])
