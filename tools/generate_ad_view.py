...
def main():
    ...
    if a.check:
        with tempfile.TemporaryDirectory(prefix="napms-ad-view-") as tmp:
            path=Path(tmp)/"application-deployment-domain.puml";path.write_text(render(m),encoding="utf-8");print(f"generated {path}")
    else:
