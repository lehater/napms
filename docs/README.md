...
- `ui/` — implementation-oriented Web UI guidance derived from accepted requirements.
...
For a new work session: root `AGENTS.md` -> `plans/active/README.md` -> current plan -> nearest scoped `AGENTS.md` -> smallest applicable Skill -> only the canonical artifacts that work package/Skill requires.
