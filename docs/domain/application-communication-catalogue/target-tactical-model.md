# Application Communication Catalogue Target Tactical Model
...
Define the Application Communication Catalogue target write/read semantics selected by ADR-012 and completed by ADR-013 while the current I27 tactical model remains runtime truth until implementation is absorbed.
...
The target model preserves the existing downstream semantic boundary through an ACC-owned compatibility projection.
...
Compatibility IDs are not target user-facing authoring identities.
...
Target reads are bounded projections rather than one whole Application tree.
...
Existing I27 Application and Component IDs remain valid and may be presented as target Definitions/Components.

Existing pre-I31 Component Deployments, DCS revisions and Deployment Resource Bindings are not automatically transformed into target Application Deployments or Interaction Definitions because required business context cannot be inferred safely.
...
- the target user mental model is represented directly in ACC;
...
- M1 implementation can proceed without transport/persistence-specific domain compromises.
