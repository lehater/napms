...
Status: `parked context roadmap; implementation gate closed`.

Date: 2026-09-13.

Resume point: **D1 — cross-context input/output contracts**.
...
This roadmap is durable future guidance for APR. It is not active execution while parked.

Canonical APR problem statement:

- `docs/domain/access-policy-realization/README.md`.

Context-roadmap lifecycle:

- `docs/process/plan-lifecycle.md`;
- `docs/engineering/roadmaps/README.md`.

When APR is resumed, revalidate upstream dependencies, select the recorded next stage and create a new active `PLAN-*.md` for that stage.
...
| D1 | cross-context input/output contracts | exact target/required/configured contracts, ownership, time, completeness and provenance | next on resume |
