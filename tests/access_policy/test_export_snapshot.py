...
from napms.access_policy.application.export_snapshot import (
...
def test_complete_snapshot_is_immutable_and_preserves_all_correlations():
    rule = active_rule()
    ...
    assert tuple(
        captured.resource_reference for captured in item.source_realizations
    ) == (ref("src-a"), ref("src-b"))
    assert tuple(
        captured.resource_reference for captured in item.destination_realizations
    ) == (ref("dst-a"),)
    assert all(captured.fact_reference for captured in item.source_realizations)
    assert all(captured.validity_reference for captured in item.source_realizations)
    assert all(captured.provenance_reference for captured in item.source_realizations)
...
def test_empty_effective_selection_produces_complete_empty_snapshot_without_catalogue_calls():
    service, application, resources = assembler({}, {})
...
def test_selected_result_without_authority_provenance_is_unavailable(authority_reference):
    service, application, resources = assembler({}, {})
...
def test_scope_drift_in_selection_fails_before_catalogue_access():
...
def test_non_effective_rule_cannot_be_reintroduced_by_snapshot_input(rule):
    service, application, resources = assembler({}, {})
...
def test_naive_selection_as_of_fails_before_catalogue_access():
    service, application, resources = assembler({}, {})
...
def test_duplicate_rule_in_selection_fails_before_catalogue_access():
    rule = active_rule()
    service, application, resources = assembler({}, {})
...
def test_application_projection_failure_prevents_snapshot_and_short_circuits_rc(
    outcome, category
):
    rule = active_rule()
    fact = ApplicationProjectionFact(outcome=outcome)
    service, application, resources = assembler(
        {rule.semantic_identity: fact},
        {},
    )
    ...
    assert result.outcome is SnapshotAssemblyOutcome.INCOMPLETE
    assert result.snapshot is None
    assert result.diagnostics[0].rule_id == rule.rule_id
    assert result.diagnostics[0].source is (
        SnapshotFactSource.APPLICATION_COMMUNICATION_CATALOGUE
    )
    assert result.diagnostics[0].category is category
...
def test_application_projection_exact_correlation_is_required(fact):
    rule = active_rule()
    service, _, resources = assembler(
        {rule.semantic_identity: fact(rule)},
        {},
    )
    ...
    assert result.outcome is SnapshotAssemblyOutcome.INCOMPLETE
    assert any(
        diagnostic.category is SnapshotFailureCategory.CORRELATION_MISMATCH
...
def test_resolved_application_projection_requires_complete_snapshot_evidence(fact):
    rule = active_rule()
    service, _, resources = assembler(
        {rule.semantic_identity: fact(rule)},
        {},
    ...
    assert result.outcome is SnapshotAssemblyOutcome.INCOMPLETE
    assert any(
        diagnostic.category is SnapshotFailureCategory.INVALID
        for diagnostic in result.diagnostics
    )
    assert resources.calls == []
...
def test_resource_realization_exact_correlation_is_required(fact):
    rule = active_rule()
    service, _, _ = assembler(
...
def test_resolved_resource_fact_requires_complete_snapshot_evidence(fact):
    rule = active_rule()
    service, _, _ = assembler(
        {rule.semantic_identity: application_fact(rule)},
        {
...
def test_one_incomplete_rule_prevents_successful_snapshot_without_silent_drop():
    first = active_rule(rule_id=1)
    second = active_rule(rule_id=2)
    first_app = application_fact(
        first,
        source=("src-first",),
    ...
    service, application, resources = assembler(
        {
            first.semantic_identity: first_app,
    ...
    assert any(
        diagnostic.rule_id == second.rule_id
...
def test_same_resource_reference_is_captured_once_per_snapshot_attempt():
    first = active_rule(rule_id=1)
    second = active_rule(rule_id=2)
    shared = ref("shared")
    service, _, resources = assembler(
    ...
    first_shared = result.snapshot.items[0].source_realizations[0]
    second_shared = result.snapshot.items[1].source_realizations[0]
    assert first_shared == second_shared


def test_port_exception_propagates_and_cannot_become_snapshot_success():
    rule = active_rule()
